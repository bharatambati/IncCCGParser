...
/**
 ...
 dependencies is generated. <p> Each category has the following
 fields: <ul> <li>A <b>catString</b>, the string representation of the
 category.<br> (Contrary to the implementation of categories in {@link
 BinaryTree BinaryTree}, these are the "real" string representations,
 not an encoded form of these).  <li>An <b>id</b> number, which
 identifies this category token.<br> Required as bookkeeping device
 for unification.  <li>A list of lexical heads <b>heads</b>, which is
 possibly null (in the case of type-raising).  <li>A <b>HeadId</b>
 number, identifying the lexical heads; also bookkeeping device for
 unification.  <li> A pointer <b>function</b> which points to the
 parent if this category is part of a complex category, or to null
 otherwise.  <li> A list of <b>unfilled dependencies</b>, which is
 only non-null if the category is an argument.  <li> A list of
 <b>filledDependencies</b>, which records the dependencies filled by
 the last application of a combinatory rule. This is used to compute
 the dependency probabilities during parsing.  </ul> A complex
 category has additionally the following fields: <ul> <li> A
 <b>result</b> category <li> An <b>argument</b> category <li> An
 integer <b>argDir</b>, indicating the directionality of the argument.
 </ul> <p> This class implements methods which <ul> <li> read in a
 categorial lexicon from a file <li> when creating a lexical category
 for a word, treat special cases of lexical categories so that
 dependencies between argument slots can be passed around.  <li>
 create new categories by performing the operations of the combinatory
 rules on existing categories <li> perform unification operations
 necessary to fill word-word dependencies.  </ul>
 */
	...
	public static CCGcat combine(CCGcat leftCat, CCGcat rightCat, String resultCatString){
		...
		if (leftCat != null && rightCat != null){
			...
			if (resultCatString.endsWith(conjFeature) || 
					(leftCat.catString().equals("conj")
							&& rightCat.matches(resultCatString)) ||
							(rightCat.catString().equals("conj")
									&& leftCat.matches(resultCatString))){
				// X[conj] --> conj X
