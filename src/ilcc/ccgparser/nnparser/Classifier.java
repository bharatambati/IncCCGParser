...
public class Classifier {
  ...
  public double[] computeScores(int[] feature, List<Integer> olist) {
    return computeScores(feature, preMap, olist);
  }

  private double[] computeScores(int[] feature, Map<Integer, Integer> preMap, List<Integer> olist) {
    double[] hidden = new double[config.hiddenSize];
    int offset = 0;
...
/*
    ...
    * */
    ...
    for (int i = 0; i < size; ++i)
      for (int j = 0; j < config.hiddenSize; ++j)
        scores[i] += W2[olist.get(i)][j] * hidden[j];
    return scores;
  }
  ...
  private double[] computeScores(int[] feature, Map<Integer, Integer> preMap) {
    ...
    /**
   * Returns the the hidden layer values.
   */
  double[] computeHidden(int[] feature) {
    double[] hidden = new double[config.hiddenSize];
    int offset = 0;
    for (int j = 0; j < feature.length; ++j) {
      int tok = feature[j];
