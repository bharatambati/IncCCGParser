 ...
 * @author ambati
 */
public class IncNNParser extends IncParser {
    ...
    public IncNNParser(Properties props) throws IOException {
    ...
    public class TempNNAgenda {
        ...
        public TempNNAgenda(PStateItem item, ArcJAction act, int[] feats, double val){
            state = item;
            action = act;
            featArray = feats;
            score = val;
    ...
    public List<Integer> getFeatures(PStateItem curState, List<CCGJTreeNode> sentNodes, List<Integer> rightPerList) {
        ...
        DepGraph depGraph = curState.getdepGraph();
        ...
        q0 = curState.getCurrentWrd() >= sentNodes.size() ? null : sentNodes.get(curState.getCurrentWrd());
        q1 = curState.getCurrentWrd()+1 >= sentNodes.size() ? null : sentNodes.get(curState.getCurrentWrd()+1);
        q2 = curState.getCurrentWrd()+2 >= sentNodes.size() ? null : sentNodes.get(curState.getCurrentWrd()+2);
        q3 = curState.getCurrentWrd()+3 >= sentNodes.size() ? null : sentNodes.get(curState.getCurrentWrd()+3);
    ...
    public double parse(List<CCGJSentence> sentences) throws IOException, Exception{
        ...
        for(CCGJSentence sent: sentences){
            ...
            srparser.sysccgDeps = output.getSysDeps();
    ...
    public PStateItem parse(CCGJSentence sent, int sentId) throws Exception {
        ...
        while(!agenda.isEmpty() && index < thres){
            ...
            for(int itemId = 0; itemId < agenda.size(); itemId++){
                ...
                int[] fArray = Ints.toArray(getFeatures(item, input, rightPerList));
                acts = getAction(item);
                ...
                if(beamSize!=1) scores = softmax(scores, olist);
